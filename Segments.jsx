...
function CitySearch({ value, onPick }) {
  ...
  const search = (text) => {
    ...
    setLoading(true);
    fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(text)}&count=8&language=en&format=json`)
      .then(r => r.json())
  ...
  const onChange = (e) => {
    ...
    tRef.current = setTimeout(() => search(v), 200);
  };
  ...
  const pick = (r) => {
    ...
    setQ(label);
    setOpen(false);
    onPick({ city: label, lat: r.latitude, lon: r.longitude, tz: r.timezone });
  };
    ...
    <div className="city-search">
      ...
      {open && (results.length > 0 || loading) && (
        <ul className="city-dropdown">
          {loading && <li className="city-loading">Searching…</li>}
          {results.map(r => (
            <li key={r.id} onMouseDown={() => pick(r)}>
              <div className="city-name">{r.name}</div>
              <div className="city-sub">{[r.admin1, r.country].filter(Boolean).join(', ')}</div>
            </li>
          ))}
        </ul>
      )}
...
function WeatherSeg({ segment, api }) {
  ...
  const [editing, setEditing] = React.useState(!segment.lat);
  const unit = segment.unit || 'F'; // 'F' or 'C'

  React.useEffect(() => {
    if (!segment.lat || !segment.lon) return;
    setErr(null);
    const tempUnit = unit === 'F' ? 'fahrenheit' : 'celsius';
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${segment.lat}&longitude=${segment.lon}&current=temperature_2m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min&temperature_unit=${tempUnit}&timezone=auto&forecast_days=6`;
    fetch(url)
      .then(r => r.json())
      .then(d => setData(d))
      .catch(e => setErr('Couldn\'t load weather'));
  }, [segment.lat, segment.lon, unit]);

  const pickCity = ({ city, lat, lon, tz }) => {
    api.update({ city, lat, lon, tz });
    setEditing(false);
  };

  const curTemp = data?.current?.temperature_2m;
  const curCode = data?.current?.weather_code;
  const [curLabel, curEmoji] = curCode != null ? wmo(curCode) : ['—', '·'];
  const daily = data?.daily;

  return (
    <TileChrome segment={segment} api={api} icon={<Ico.weather width="14" height="14"/>}
      toolbar={segment.lat ? (
        <button className="icon-btn" title="Change city" onClick={() => setEditing(e => !e)}>
          <Ico.gear width="13" height="13"/>
        </button>
      ) : null}>
      {editing ? (
        <div className="weather-edit">
          <div className="edit-label">City</div>
          <CitySearch value={segment.city} onPick={pickCity}/>
          <div className="edit-label">Units</div>
          <div className="seg-row small">
            <button className={`seg-btn ${unit==='F'?'on':''}`} onClick={() => api.update({ unit: 'F' })}>°F</button>
            <button className={`seg-btn ${unit==='C'?'on':''}`} onClick={() => api.update({ unit: 'C' })}>°C</button>
          </div>
          {segment.lat && <button className="btn-ghost small" onClick={() => setEditing(false)}>Done</button>}
        </div>
      ) : err ? (
        ...
        <div className="weather-body">
          <div className="weather-top">
            <div>
              <div className="weather-big">{Math.round(curTemp)}°<span className="weather-unit">{unit}</span></div>
              <div className="weather-cond">{curLabel}</div>
              <div className="weather-city">{segment.city}</div>
            </div>
            <div className="weather-emoji">{curEmoji}</div>
          </div>
          {daily && (
            <div className="weather-mini">
              {daily.time.slice(1, 6).map((d, i) => {
                const [, em] = wmo(daily.weather_code[i+1]);
                ...
                return (
                  <div key={d} className="weather-day">
                    <div className="wd-l">{day}</div>
                    <div className="wd-emoji">{em}</div>
                    <div className="wd-t">{hi}°<span className="wd-lo">{lo}°</span></div>
                  </div>
                );
              })}
            </div>
