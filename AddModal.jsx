      ...
      return (
        <>
          <div className="form-row"><label>City</label>
            <CitySearch value={form.city} onPick={({city, lat, lon, tz}) => set({ city, lat, lon, tz })}/>
          </div>
          <div className="form-row"><label>Units</label>
            <div className="seg-row small">
              <button type="button" className={`seg-btn ${(form.unit||'F')==='F'?'on':''}`} onClick={() => set({ unit: 'F' })}>°F</button>
              <button type="button" className={`seg-btn ${form.unit==='C'?'on':''}`} onClick={() => set({ unit: 'C' })}>°C</button>
            </div>
          </div>
        </>
      );
...
function typeExtras(type, form) {
  switch (type) {
    ...
    case 'weather': return { city: form.city || '', lat: form.lat, lon: form.lon, tz: form.tz, unit: form.unit || 'F' };
